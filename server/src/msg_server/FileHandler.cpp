...
void CFileHandler::HandleClientFileRequest(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    if (pFileConn)
    {
        ...
        {
            pFileConn->SendPdu(SID_OTHER, CID_OTHER_FILE_TRANSFER_REQ, msg2);
        }
        else //IM::BaseDefine::FILE_TYPE_ONLINE
        {
            ...
            else//无对应用户的pc登录状态,向route_server查询状态
            {
                ...
                if (route_conn)
                {
                    ...
                    route_conn->SendPdu(SID_BUDDY_LIST, CID_BUDDY_LIST_USERS_STATUS_REQUEST, msg3);
                }
            }
        }
    }
    else
    {
        ...
        pMsgConn->SendPdu(SID_FILE, CID_FILE_RESPONSE, msg2);
    }
}
...
void CFileHandler::HandleClientFileHasOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    else
    {
        ...
        pMsgConn->SendPdu(SID_FILE, CID_FILE_HAS_OFFLINE_RES, msg);
    }
}
...
void CFileHandler::HandleClientFileAddOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    if (pFileConn)
    {
        ...
        if (pConn != NULL) 
        {
            pConn->SendPdu(SID_FILE, CID_FILE_NOTIFY, msg2);
        }
    }
}
